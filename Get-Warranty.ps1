#function Get-Warranty {
	...
	foreach($Computer in $ComputerName) {
		...
		if ($computerIsUp) {
			...
			$serviceTag = $bios | Select-Object -ExpandProperty SerialNumber
			$compName   = $bios | Select-Object -ExpandProperty __SERVER
			$model      = $system | Select-Object -ExpandProperty Model
